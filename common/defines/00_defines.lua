...
NGame = {
	...
	GAME_SPEED_SECONDS = { 2.0, 1.0, 0.5, 0.25, 0.0 }, -- game speeds for each level. Must be 5 entries with last one 0 for unbound
	...
	EVENT_TIMEOUT_DEFAULT = 24,						-- Default days before an event times out if not scripted
	MISSION_REMOVE_FROM_INTERFACE_DEFAULT = 24,		-- Default days before a mission is removed from the interface after having failed or completed
...
NDiplomacy = {
	...
	TRUCE_PERIOD_AFTER_KICKING_FROM_FACTION = 50,				-- Truce period after kicking someone from faction in days.
	...
	TRUCE_BREAK_COST_PP = 250,						-- Base cost in PP of breaking a truce by joining a war or accepting a call to war ( you cannot declare war yourself until the truce if up ), this is then multiplied by the time left on the truce ( so once half the truce is up it only cost 50% of this )
	...
	AUTONOMY_LEVEL_CHANGE_PP_COST_BASE = 100,		-- Base cost of changing level of autonomy
	AUTONOMY_LEVEL_CHANGE_PP_ANNEX = 100,			-- Annexation cost
	AUTONOMY_LEVEL_CHANGE_PP_FREE = 250,			-- Break free cost
	...
	CAPITAL_CAPITULATE_BONUS_SCORE = 250,			-- extra bonus when deciding who to capitulate to (applied to capital holder)
	...
	ASSUME_FACTION_LEADERSHIP_MIN_MANPOWER_RATIO = 2.5,		-- The minimum ratio of manpower that a country must have compared to the current leader in order to assume leadership.
	ASSUME_FACTION_LEADERSHIP_MIN_FACTORY_RATIO = 2,		-- The minimum ratio of factories that a country must have compared to the current leader in order to assume leadership.
	ASSUME_FACTION_LEADERSHIP_COOLDOWN_DAYS = 90,			-- Number of days after formation of faction or change in leadership before another country is allowed to assume leadership.
	ASSUME_FACTION_SPYMASTER_COOLDOWN_DAYS = 90,			-- Number of days after change of Spy Master before another country is allowed to become Spy Master.
	FACTION_LEADERSHIP_CHANGE_ALERT_THRESHOLD = 0.5,		-- Threshold for showing an alert when a faction member is close to being able to assume leadership of the faction that a player currently leads.
